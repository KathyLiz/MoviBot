...
ws_miMovistar.prototype.ws_gui_idm_handle_json = function(){
	if (this.ws_gui_idm.unirest.readyState === 4) {
		if (this.ws_gui_idm.unirest.statusCode === 200) {
			var json_data = this.ws_gui_idm.unirest.responseText;
			
			try{
				
				this.ws_gui_idm.resultado = eval("(" + json_data + ")");				
				console.log(this.ws_gui_idm.resultado);
				...
				if( this.ws_gui_idm.resultado[0]==="ERROR")
			 	...
			 	console.log(err)
				console.log(this.ws_gui_idm.unirest);
					error = {};
